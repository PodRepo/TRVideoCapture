...
//                                              [NSNumber numberWithInt:4], AVVideoPixelAspectRatioHorizontalSpacingKey,
...
//                                   nil];
//    self.movieFileOutput.outputSettings = videoSettings;
...
- (void)cancelCurrentVideoRecording
{
    [self stopCountDurTimer];
    [_movieFileOutput stopRecording];
    _cancel = true;
    ...
    if (_cancel) {
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([_delegate respondsToSelector:@selector(videoRecorder:didFinishRecordingToOutPutFileAtURL:duration:totalDur:error:)]) {
                [_delegate videoRecorder:self didFinishRecordingToOutPutFileAtURL:nil duration:0 totalDur:0 error:[NSError errorWithDomain:@"cancel" code:0 userInfo:nil]];
            }
        });
        return;
    }
...
//        AVVideoPixelAspectRatioVerticalSpacingKey: @4,
//        },
    ...
    [encoder exportAsynchronouslyWithCompletionHandler:^
     {
         if (encoder.status == AVAssetExportSessionStatusCompleted)
         {
             ...
             dispatch_async(dispatch_get_main_queue(), ^{
                 if ([_delegate respondsToSelector:@selector(videoRecorder:didFinishRecordingToOutPutFileAtURL:duration:totalDur:error:)]) {
                     [_delegate videoRecorder:self didFinishRecordingToOutPutFileAtURL:finalURL duration:_currentVideoDur totalDur:_totalVideoDur error:error];
                 }
             });
         }
         else if (encoder.status == AVAssetExportSessionStatusCancelled)
         {
             ...
             dispatch_async(dispatch_get_main_queue(), ^{
                 if ([_delegate respondsToSelector:@selector(videoRecorder:didFinishRecordingToOutPutFileAtURL:duration:totalDur:error:)]) {
                     [_delegate videoRecorder:self didFinishRecordingToOutPutFileAtURL:nil duration:0 totalDur:0 error:[NSError errorWithDomain:@"cancel trans" code:0 userInfo:nil]];
                 }
             });
         }
         else
         {
             dispatch_async(dispatch_get_main_queue(), ^{
                 if ([_delegate respondsToSelector:@selector(videoRecorder:didFinishRecordingToOutPutFileAtURL:duration:totalDur:error:)]) {
                      [_delegate videoRecorder:self didFinishRecordingToOutPutFileAtURL:nil duration:0 totalDur:0 error:[NSError errorWithDomain:@"error" code:0 userInfo:nil]];
                 }
             });
